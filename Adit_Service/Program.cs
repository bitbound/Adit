...
namespace Adit_Service
{
    class Program
    {
        static void Main()
        {
            ...
            {
                AditService.StartInteractive().Wait();
            }
            else if (args.Exists(str => str.ToLower() == "-install"))
            {
                try
                {
                    var di = Directory.CreateDirectory(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles) + @"\Adit\");
